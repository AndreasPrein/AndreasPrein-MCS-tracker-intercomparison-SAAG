...
def jetstream_tracking(
    ...
    if breakup == 'breakup':
        ...
        jet_objects = watersheding(jet_objects,
...
#             ii = ii + 1
    ...
    if breakup == 'breakup':
        ...
        #                1)
        threshold=1
        min_dist=int((100 * 10**3)/Gridspacing)
        tb_masked = np.copy(Csmooth)
...
def watersheding(label_matrix,  # 2D or 3D matrix with labeled objects [np.array]
    ...
    local_maxi = peak_local_max(
        distance, indices=False, footprint=conection, labels=label_matrix,
...
def watershed_2d_overlap(
    anom_sel_masked, # 3D numpy array [time,lat,lon] with anomaly field that will be used for the watersheding.
    mask_sel,        # 3D numpy array [time,lat,lon] with objects set to True and background set to False
    Gridspacing,     # average grid spacing in m
    dT,              # data interval [h]
    connectLon,      # 1 -> connect objects on lon edges (i.e., date line)
    min_dist=10,     # index setting the minimum distance between local maxima [m]
    threshold=1,     # absolute threshold for differentating local maxima [units of anomaly]
    mintime = 0,     # minimum time that an object has to live
    ):

    # Perform watershedding in 2D
    import numpy as np
    from skimage.segmentation import watershed
    from skimage.feature import peak_local_max
    ...
    # Anomalies have to be possitive!
    if np.max(anom_sel_masked) <= 0:
        anom_sel_masked = anom_sel_masked[:]*-1

    distance = anom_sel_masked[:]
    # smooth field for better maxima detection
    distance = smooth_uniform(distance,
                               4,
                               int(750/(Gridspacing/1000.))*2)

    # derive local maxima on 2D snap shots
    local_maxi = np.zeros((distance.shape), dtype=np.bool); local_maxi[:] = False
    markers = np.zeros((distance.shape), dtype=np.int32)
    labels = np.zeros((distance.shape), dtype=np.int32)
    for jj in range(distance.shape[0]):
        local_maxi[jj,:,:] = peak_local_max(
            distance[jj,:], indices=False, footprint=np.ones((3, 3)), labels=label_matrix[jj,:],
            min_distance=min_dist, threshold_abs=threshold)
        markers[jj,:] = ndimage.label(local_maxi[jj,:,:])[0]
        labels[jj,:] = watershed(-distance[jj,:], markers[jj,:], mask=label_matrix[jj,:], connectivity = np.ones((3, 3)))
    ...
    for tt in tqdm(range(objects_watershed.shape[0])):
        ...
        if tt == 0:
            ...
            try:
                ob_loc_t0 = np.array(ob_loc_t0)[valid]
            except:
                continue
